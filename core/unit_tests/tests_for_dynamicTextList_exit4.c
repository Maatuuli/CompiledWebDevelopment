...
executeTest1(void)
{
    char* titleFromTest = "#04 Push a NULL parameter as text to a listTypeForDynamicText which is not allowed!";

    printf("[SUCCESS] %s\n", titleFromTest);
    
    struct listForDynamicText* list1 = newListForDynamicText(__FILE__, __LINE__);
    
    pushElementInListForDynamicText(&list1, NULL, __FILE__, __LINE__);
}
