...
executeTest1(void)
{
    char* titleFromTest = "#01 Get substring from a NULL parameter is not allowed!";
    printf("[SUCCESS] %s\n", titleFromTest);
    
    struct dynamicText* var2 = allocateDynamicText(__FILE__, __LINE__);

    getSubstringInBytesFromDynamicText(NULL, 0, 5, &var2, __FILE__, __LINE__);
}
...
{
    printf("\n");
    printf("TEST SUITE: getSubstringInBytesFromDynamicText function => Not allowed test cases ending with exit() \n");
