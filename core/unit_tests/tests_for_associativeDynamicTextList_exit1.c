...
executeTest1(void)
{
    char* titleFromTest = "#01 Empty check on NULL parameter is not allowed!";
    printf("[SUCCESS] %s\n", titleFromTest);
    
    isListEmptyForAssociativeDynamicText(NULL, __FILE__, __LINE__);
}
...
{
    printf("\n");
    printf("TEST SUITE: associative_dynamic_text_list.c file => Not allowed test cases ending with exit() \n");
