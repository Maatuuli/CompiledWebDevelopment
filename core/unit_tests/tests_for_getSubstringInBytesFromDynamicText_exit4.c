...
executeTest1(void)
{
    char* titleFromTest = "#04 Start index is greater-equal than the maximum length of text which is not allowed!";
    printf("[SUCCESS] %s\n", titleFromTest);
    ...
    getSubstringInBytesFromDynamicText(&var1, customStrlen(getBytesPointerFromDynamicText(&var1)) + 50, customStrlen(getBytesPointerFromDynamicText(&var1)) + 150, &var2, __FILE__, __LINE__);
}
