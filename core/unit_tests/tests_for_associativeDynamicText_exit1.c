...
executeTest1(void)
{
    char* titleFromTest = "#01 Parameter for text is NULL which is not allowed!";
    printf("[SUCCESS] %s\n", titleFromTest);
    
    allocateAssociativeDynamicTextByKey(NULL, __FILE__, __LINE__);
}
...
{
    printf("\n");
    printf("TEST SUITE: associative_dynamic_text.c file => Not allowed test cases ending with exit() \n");
