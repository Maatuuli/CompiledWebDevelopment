...
executeTest1(void)
{
    char* titleFromTest = "#16 Get element by key from empty parameter for key is not allowed!";
    printf("[SUCCESS] %s\n", titleFromTest);
    ...
    getElementByKeyFromListForAssociativeDynamicText(&list1, "", __FILE__, __LINE__);
}
