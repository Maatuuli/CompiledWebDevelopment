...
executeTest1(void)
{
    char* titleFromTest = "#17 Get element by key from non-existing key is not allowed!";
    printf("[SUCCESS] %s\n", titleFromTest);
    ...
    getElementByKeyFromListForAssociativeDynamicText(&list1, "name2", __FILE__, __LINE__);
}
