...
executeTest1(void)
{
    char* titleFromTest = "#06 End index is greater-equal than the maximum length of text which is not allowed!";
    printf("[SUCCESS] %s\n", titleFromTest);
    ...
    getSubstringInBytesFromDynamicText(&var1, 5, customStrlen(getBytesPointerFromDynamicText(&var1)) + 5, &var2, __FILE__, __LINE__);
}
