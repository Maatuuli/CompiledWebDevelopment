...
executeTest1(void)
{
    char* titleFromTest = "#10 Get a list element from a NULL pointer for listTypeForDynamicText which is not allowed!";
    printf("[SUCCESS] %s\n", titleFromTest);
    
    struct listForDynamicText* list1 = newListForDynamicText(__FILE__, __LINE__);
    
    getElementByIndexFromListForDynamicText(&list1, 0, __FILE__, __LINE__);
}
