...
executeTest1(void)
{
    char* titleFromTest = "#14 Get element by key from NULL pointer parameter for list is not allowed!";
    printf("[SUCCESS] %s\n", titleFromTest);
    
    struct listForAssociativeDynamicText* list1 = newListForAssociativeDynamicText(__FILE__, __LINE__);
    
    getElementByKeyFromListForAssociativeDynamicText(&list1, "name1", __FILE__, __LINE__);
}
