...
executeTest1(void)
{
    char* titleFromTest = "#03 Check if key exists on NULL parameter for key is not allowed!";
    printf("[SUCCESS] %s\n", titleFromTest);
    
    struct listForAssociativeDynamicText* list1 = newListForAssociativeDynamicText(__FILE__, __LINE__);
    
    checkIfKeyExistsInListForAssociativeDynamicText(&list1, NULL, __FILE__, __LINE__);
}
