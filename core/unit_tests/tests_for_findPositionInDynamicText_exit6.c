...
executeTest1(void)
{
    char* titleFromTest = "#06 Offset for start index is greater-equal than length of text which is not allowed!";
    printf("[SUCCESS] %s\n", titleFromTest);
    ...
    findPositionInDynamicText(&var1, "abc", customStrlen(getBytesPointerFromDynamicText(&var1)), __FILE__, __LINE__);
}
