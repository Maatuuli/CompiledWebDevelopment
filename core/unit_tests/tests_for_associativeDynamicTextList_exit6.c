...
executeTest1(void)
{
    char* titleFromTest = "#06 Add element with NULL parameter for key is not allowed!";
    printf("[SUCCESS] %s\n", titleFromTest);
    
    struct listForAssociativeDynamicText* list1 = newListForAssociativeDynamicText(__FILE__, __LINE__);
    
    addElementByKeyInListForAssociativeDynamicText(&list1, NULL, "value1", __FILE__, __LINE__);
}
