...
executeTest1(void)
{
    char* titleFromTest = "#15 Get element by key from NULL parameter for key is not allowed!";
    printf("[SUCCESS] %s\n", titleFromTest);
    ...
    getElementByKeyFromListForAssociativeDynamicText(&list1, NULL, __FILE__, __LINE__);
}
