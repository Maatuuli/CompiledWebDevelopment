...
executeTest1(void)
{
    char* titleFromTest = "#08 Add element with NULL parameter for value is not allowed!";
    printf("[SUCCESS] %s\n", titleFromTest);
    
    struct listForAssociativeDynamicText* list1 = newListForAssociativeDynamicText(__FILE__, __LINE__);
    
    addElementByKeyInListForAssociativeDynamicText(&list1, "name1", NULL, __FILE__, __LINE__);
}
