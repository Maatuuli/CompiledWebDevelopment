...
appendDynamicText(struct DynamicText** element, char* text, int* errorNumber, char* filename, int lineNumber)
{
    ...
    if (maxLengthForCopy > (*element)->amountOfBytes)
    {
        ...
        /* There is a selfmade limit for maximum amount of allocated text. */
    ...
    // @TODO: Normally There should be no need for the additional null termination, or?
    // Replace assignment with assert:
    // assert('\0' == (*element)->bytes[(*element)->amountOfBytes - 1]);
